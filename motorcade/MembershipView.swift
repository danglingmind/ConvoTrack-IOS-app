...
struct MembershipView: View {
    ...
    @State private var selectedPlan: String = "danglingmind.motorcade.membership.yearly"
    ...
    private var selectedPlanDisclosure: String {
        guard let product = store.products.first(where: { $0.id == selectedPlan }) else {
            return "Motorcade Pro is an auto-renewable subscription."
        }
        let period = selectedPlan.contains("yearly") ? "year" : "month"
        return "Motorcade Pro is a \(product.displayPrice)/\(period) auto-renewable subscription."
    }
