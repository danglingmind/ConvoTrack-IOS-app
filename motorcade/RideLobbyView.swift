...
struct RideLobbyView: View {
    ...
    private var deepLinkURL: URL? {
        ...
        return URL(string: "motorcade://join/\(inviteCode)")
    }

    private var shareItems: [Any] {
        guard let url = deepLinkURL else {
            return ["Join my Motorcade ride!"]
        }
        return [url, "Join my Motorcade ride! Tap to open in Motorcade: motorcade://join/\(inviteCode)"]
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 32) {
                ...
                Text("JOIN MOTORCADE").font(.headlineMd).foregroundColor(Color.primaryFixed).tracking(2)
