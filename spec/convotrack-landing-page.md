# vynl.in/convotrack — Landing Page Spec

This page is linked from every ride share QR code generated in the ConvoTrack app.
...
1. **Deep-link existing ConvoTrack users** directly into the app (via a universal link / app clip).
2. **Convert new users** — show them what ConvoTrack is and get them to download it.
...
|---|---|
| iPhone user with ConvoTrack installed | Opens the app directly (universal link) |
| iPhone user without ConvoTrack | Sees a download prompt → App Store |
...
### 1. Hero
- App icon + "CONVOTRACK" wordmark (match app branding: dark background `#131313`, lime accent `#caf300`)
...
- "You were invited to a convotrack"
...
- Turn-by-turn navigation that keeps the convotrack together
...
- Associate with bundle ID `danglingmind.convotrack`
- Path to handle: `/convotrack*`
...
```json
{
  "applinks": {
    ...
    "details": [
      {
        "appIDs": ["<TEAM_ID>.danglingmind.convotrack"],
        "components": [
          { "/": "/convotrack*" }
        ]
      }
...
### iOS Behavior
- If app is installed: tapping the link from Messages/Safari opens ConvoTrack directly
...
MVP checklist:
- [ ] Page loads at `https://vynl.in/convotrack`
