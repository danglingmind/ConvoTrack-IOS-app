# vynl.in/convoy — Landing Page Spec

This page is linked from every ride share QR code generated in the Convoy app.
...
1. **Deep-link existing Convoy users** directly into the app (via a universal link / app clip).
2. **Convert new users** — show them what Convoy is and get them to download it.
...
|---|---|
| iPhone user with Convoy installed | Opens the app directly (universal link) |
| iPhone user without Convoy | Sees a download prompt → App Store |
...
### 1. Hero
- App icon + "CONVOY" wordmark (match app branding: dark background `#131313`, lime accent `#caf300`)
...
- Turn-by-turn navigation that keeps the convoy together
...
- Associate with bundle ID `danglingmind.convoy`
- Path to handle: `/convoy*`
...
```json
{
  "applinks": {
    ...
    "details": [
      {
        "appIDs": ["<TEAM_ID>.danglingmind.convoy"],
        "components": [
          { "/": "/convoy*" }
        ]
      }
...
### iOS Behavior
- If app is installed: tapping the link from Messages/Safari opens Convoy directly
...
MVP checklist:
- [ ] Page loads at `https://vynl.in/convoy`
